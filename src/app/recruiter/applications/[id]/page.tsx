...
import {
  ChevronLeft, FileText, Info, RefreshCw, ChevronDown, ChevronUp,
  Calendar, Clock, X, CheckCircle,
} from "lucide-react";
...
function ScoreRing({ score }: { score: number }) {
  ...
  return (
    <svg width="100" height="100" className="rotate-[-90deg]">
      ...
      <text x="50" y="56" textAnchor="middle" style={{ rotate: "90deg", transformOrigin: "50px 50px", fontSize: 20, fontWeight: 700, fill: color }}>
        {score}
      </text>
    </svg>
  );
...
export default function ApplicationDetail() {
  ...
  const [preferredDt, setPreferredDt] = useState("");
  ...
  useEffect(() => {
    ...
    user.getIdToken().then(async (token) => {
      ...
      const [appRes, calRes] = await Promise.all([
        fetch(`/api/applications/${params.id}`, { headers }),
        fetch("/api/calendar/status", { headers }),
      ]);
      if (appRes.status === 404 || appRes.status === 403) { setNotFound(true); return; }
      const data: DetailData = await appRes.json();
      setDetail(data);
  ...
  async function scheduleInterview() {
    ...
    try {
      const token = await user.getIdToken();
      const body: { applicationId: string; preferredDateTime?: string } = { applicationId: params.id };
      if (scheduleMode === "custom" && preferredDt) {
        body.preferredDateTime = new Date(preferredDt).toISOString();
      }
      ...
      if (!res.ok) {
        ...
        setSchedError(d.error ?? "Scheduling failed");
        return;
      }
      const { startTime } = await res.json();
      setLocalApp((a) => a && { ...a, status: "interview_scheduled", scheduledAt: startTime });
    } finally {
      setScheduling(false);
    }
  }
  ...
  async function cancelInterview() {
    ...
    try {
      ...
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
        setSchedError(d.error ?? "Cancellation failed");
        return;
      }
      setLocalApp((a) => a && { ...a, status: "approved", scheduledAt: null, calendarEventId: null });
    } finally {
      setCancelling(false);
    }
  }
  ...
  const isApproved = localApp.status === "approved";
  const isScheduled = localApp.status === "interview_scheduled";
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      ...
      {(isApproved || isScheduled) && (
        <div className={`rounded-xl shadow-sm mb-6 overflow-hidden border ${
          isScheduled ? "bg-emerald-50 border-emerald-200" : "bg-white border-gray-200"
        }`}>
          {isScheduled ? (
            <div className="p-6">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-emerald-900">Interview Scheduled</p>
                    {localApp.scheduledAt && (
                      <p className="text-sm text-emerald-700 mt-0.5 flex items-center gap-1.5">
                        <Calendar size={13} />
                        {new Date(localApp.scheduledAt).toLocaleDateString(undefined, {
                          weekday: "long", year: "numeric", month: "long", day: "numeric",
                        })}
                        <Clock size={13} className="ml-1" />
                        {new Date(localApp.scheduledAt).toLocaleTimeString(undefined, {
                          hour: "2-digit", minute: "2-digit",
                        })}
                      </p>
                    )}
                    <p className="text-xs text-emerald-600 mt-1">
                      Google Calendar invite sent to {candidate?.email}
                    </p>
                  </div>
                </div>
                <button
                  ...
                  disabled={cancelling}
                  className="flex items-center gap-1.5 text-xs font-medium text-red-500 hover:text-red-700 transition-colors disabled:opacity-50 shrink-0"
                >
                  <X size={13} />
              ...
              {schedError && (
                <p className="mt-3 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{schedError}</p>
              )}
            </div>
          ) : (
            <div className="p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs bg-indigo-100 text-indigo-700 font-semibold px-2 py-0.5 rounded-full">Approved</span>
              </div>

              {calConnected === false ? (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                  <p className="text-sm font-medium text-amber-800 mb-1">Google Calendar not connected</p>
                  <p className="text-xs text-amber-700 mb-3">
                    Connect your Google Calendar to send interview invites automatically.
                  </p>
                  <Link href="/recruiter/settings"
                    className="inline-block text-xs font-semibold text-white bg-amber-500 hover:bg-amber-600 px-3 py-1.5 rounded-lg transition-colors">
                ...
                <>
                  ...
                  {scheduleMode === "auto" ? (
                    <p className="text-xs text-gray-500 mb-4">
                      BestHire will find the next available slot on your Google Calendar (weekdays at 10:00, 14:00, or 16:00, at least 48 hours from now) and send invites to both parties.
                    </p>
                  ) : (
                    <div className="mb-4">
                      ...
                      <input
                        ...
                        onChange={(e) => setPreferredDt(e.target.value)}
                      ...
                      <p className="text-xs text-gray-400 mt-1">
                        Minimum 48 hours from now. Google Calendar invites sent automatically.
                      </p>
                    </div>
                  )}

                  {schedError && (
                    <p className="mb-3 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{schedError}</p>
                  )}

                  <button
                    onClick={scheduleInterview}
                    disabled={scheduling || (scheduleMode === "custom" && !preferredDt)}
                    className="w-full py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white font-semibold text-sm rounded-xl shadow-sm disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                  >
                    <Calendar size={15} />
                    {scheduling ? "Scheduling…" : "Send Interview Invite"}
                  </button>
                </>
              )}
            </div>
      ...
      <div className="bg-white border rounded-xl shadow-sm mb-6 overflow-hidden">
        <button onClick={toggleAnalysis}
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs bg-indigo-100 text-indigo-700 font-semibold px-2 py-0.5 rounded-full">Recruiter View</span>
          </div>
          <div className="flex items-center gap-2">
            {analysisLoading && <RefreshCw size={14} className="text-indigo-500 animate-spin" />}
            {analysisOpen ? <ChevronUp size={18} className="text-gray-400" /> : <ChevronDown size={18} className="text-gray-400" />}
          </div>
        </button>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500">
                      {analysis.fitScore >= 70 ? "Strong match — candidate meets or exceeds most requirements."
                        : analysis.fitScore >= 40 ? "Partial match — some gaps but worth considering."
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {analysis.skills.matched.length > 0 && (
                      <div>
                        <p className="text-xs text-gray-400 mb-1.5">✓ Matched <span className="font-semibold text-emerald-600">{analysis.skills.matched.length}</span> required skills</p>
                        <div className="flex flex-wrap gap-1.5">
                          {analysis.skills.matched.map((s) => (
                            <span key={s} className="text-xs bg-emerald-50 text-emerald-700 border border-emerald-200 px-2.5 py-0.5 rounded-full font-medium">✓ {s}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {analysis.skills.missing.length > 0 && (
                      <div>
                        <p className="text-xs text-gray-400 mb-1.5">✗ Missing <span className="font-semibold text-red-500">{analysis.skills.missing.length}</span> required skills</p>
                        <div className="flex flex-wrap gap-1.5">
                          {analysis.skills.missing.map((s) => (
                            <span key={s} className="text-xs bg-red-50 text-red-600 border border-red-200 px-2.5 py-0.5 rounded-full font-medium">✗ {s}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {analysis.skills.extra.length > 0 && (
                      <div>
                        <p className="text-xs text-gray-400 mb-1.5">+ <span className="font-semibold text-gray-600">{analysis.skills.extra.length}</span> additional skills</p>
                        <div className="flex flex-wrap gap-1.5">
                          {analysis.skills.extra.slice(0, 8).map((s) => (
                            <span key={s} className="text-xs bg-gray-100 text-gray-500 px-2.5 py-0.5 rounded-full">{s}</span>
                          ))}
                          {analysis.skills.extra.length > 8 && <span className="text-xs text-gray-400">+{analysis.skills.extra.length - 8} more</span>}
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm text-gray-600"><span className="font-bold text-gray-900">{analysis.experience.candidate}y</span> of {analysis.experience.required}y required</span>
                      <StatusPill status={analysis.experience.status} />
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div className={`h-2 rounded-full ${analysis.experience.status === "below" ? "bg-red-400" : "bg-emerald-500"}`}
                        style={{ width: `${Math.min(100, (analysis.experience.candidate / Math.max(analysis.experience.required, 1)) * 100)}%` }} />
                    </div>
                    {analysis.experience.gap > 0 && <p className="text-xs text-emerald-600 mt-1">+{analysis.experience.gap}y over requirement</p>}
                    {analysis.experience.gap < 0 && <p className="text-xs text-red-500 mt-1">{Math.abs(analysis.experience.gap)}y short</p>}
                  </div>
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="space-y-2">
                      {analysis.workHistory.map((w, i) => (
                        <div key={i} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                          ...
                          <span className="text-xs font-semibold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">{w.years}y</span>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <div className="bg-white border rounded-xl p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-3">
          {canDecide && (
            <>
              <button onClick={() => decide("approved")} disabled={decisionLoading}
              ...
              <button onClick={() => decide("rejected")} disabled={decisionLoading}
          ...
          {!canDecide && (
            <div className="flex items-center gap-3">
              <span className={`text-sm font-semibold px-3 py-1 rounded-lg ${
                localApp.status === "approved" || localApp.status === "interview_scheduled"
                  ? "bg-green-100 text-green-800"
                  : "bg-red-100 text-red-800"
              }`}>
                {localApp.status === "interview_scheduled"
                  ? "Interview Scheduled"
                  : localApp.decision === "approved" ? "Approved" : "Rejected"}
              </span>
              {canUndo && localApp.decidedAt && localApp.status !== "interview_scheduled" && (
                <UndoCountdown
